...
async def upload_photo(file: UploadFile = File(...), hint: Optional[str] = Form(None)):
    """
    Direct photo upload endpoint: saves file and executes Rekognition analysis
    returning detected labels and confidence scores immediately.
    """
    ...
    # Analyze via Amazon Rekognition or intelligent vision engine
    analysis = analyze_image_bytes(contents, filename_hint=(hint or file.filename), category_hint=(hint or ""))
...
def subscribe_alert(payload: SubscriptionCreate):
    ...
    save_data()
    return {
        "message": f"Successfully subscribed {payload.endpoint} to emergency broadcasts via {payload.protocol.upper()}",
        "subscription": sub
    }


# Mount static React frontend build if dist exists
